...
const Steering::ReturnValue FollowNodePath::RetreatCurrentNode(Actor &self, const Vector &goalPosition)
{
	...
	if ( !GetPreviousNode(GetCurrentNode() )->GetIsJumpNode() )
	{
		...
		{
			//hzm gameupdate chrissstrahl - print out more information than before
			int i;
			str ownerModel = self.model;
			str tikiName;
			for ( i = 0; i < ownerModel.length(); i++ ){
				if ( ownerModel[i] == '/' ){
					tikiName = "";
				}else{
					tikiName += ownerModel[i];
				}
			}
			//hzm gameupdate chrissstrahl - this is so anoying, any useless on stockmaps, so have it disabled on stock maps
			if ( !game.isStandardLevel ) {
				gi.WDPrintf( "Couldn't complete path, building new one for: '$%s' %s\n" , self.TargetName() , tikiName.c_str() );
			}
			//hzm eof
