...
{
	//hzm gameupdate chrissstrahl - do not take spectators in account
	if ( target->entnum < maxclients->integer ){
		if ( multiplayerManager.isPlayerSpectator( ( Player * )target ) ){
			return false;
		}
	}
	//hzm eof
	...
	//hzm coop mod chrissstrahl - debug
	//gi.Printf( va( "CombatSubsystem::_traceHitTarget %i\n" , target->entnum ) );
