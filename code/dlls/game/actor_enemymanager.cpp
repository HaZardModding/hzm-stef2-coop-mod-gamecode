...
EntityPtr EnemyManager::GetCurrentEnemy()
{
	if ( act->forcedEnemy )
	{
		if ( act->forcedEnemy->health <= 0.0f )
		{
...
void EnemyManager::SetCurrentEnemy( Entity *enemy )
{
	if ( enemy ){
		...
		//hzm debug
		//gi.Printf( va( "EnemyManager::SetCurrentEnemy: %i\n" , enemy->entnum ) );
	}
	else
	{
		_lockedOnCurrentEnemy = false;
...
void EnemyManager::FindHighestHateEnemy()
{
	...
	//hzm gameupdate chrissstrahl - pick the closest player and best to attack player from them all
	...
	for ( int i = _hateList.NumObjects() ; i > 0 ; i-- )
	{
		...
		if ( listIndex.enemy->isSubclassOf( Sentient ) && !act->GetActorFlag(ACTOR_FLAG_UPDATE_HATE_WITH_ATTACKERS) )
		{
			...
			//hzm debug
			//gi.Printf( va( "EnemyManager::FindHighestHateEnemy: ENTITY[%i]\n" , listIndex.enemy->entnum ) );
		}
...
void EnemyManager::FindClosestEnemy()
{
	...
	for ( int i = 1 ; i <= _hateList.NumObjects() ; i++ )
	{
		...
		//hzm chrissstrahl - debug
		//Entity *eee = (Entity *)listIndex.enemy;
		//gi.Printf( va( "EnemyManager::FindClosestEnemy: ENTITY[%i]\n" , eee->entnum ) );
		//hzm eof
		...
		//hzm chrissstrahl - debug
		//gi.Printf( va( "EnemyManager::FindClosestEnemy: ENTITY[%i] SUCESS\n" , eee->entnum ) );
		//hzm eof
