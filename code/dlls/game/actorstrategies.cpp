...
void ActorThink::UpdateBossHealth( Actor &actor )
{
	...
	str sName = actor.getName();//hzm edit

	//hzm gameupdate chrissstrahl - allow custom boss name
	int i;
	...
	for ( i = 1; i < 5; i++ ){
		...
		entityData = NULL;
		sEntityName = "";
		entityData = eActor->entityVars.GetVariable( va( "uservar%d" , i ) );
		
		//if empty skip to next uservar
		if ( entityData == NULL ){
			continue;
		}

		sEntityName = entityData->stringValue();
		if ( !Q_stricmpn( "name" , sEntityName , 4 ) ){
	...
	gi.cvar_set( "bossname" , sName.c_str() );//hzm edit
}
