...
Item *AmmoEntity::ItemPickup( Entity *other, qboolean add_to_inventory, qboolean )
{
	...
	//[b607] chrissstrahl - this was moved here and optimised - item is picked up if any player could use it
	...
	if (game.coop_isActive){
		...
		for (int i = 0; i < maxclients->integer; i++){
			...
			if (coopPlayer && coopPlayer->client && coopPlayer->isSubclassOf(Player) && !multiplayerManager.isPlayerSpectator(coopPlayer) && !(coopPlayer->edict->svflags & SVF_BOT)){
