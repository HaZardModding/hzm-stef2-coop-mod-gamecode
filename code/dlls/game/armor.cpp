...
AdaptiveArmor::AdaptiveArmor()
{
	...
	//hzm gamefix chrissstrahl - borg do no longer adapt to batleth and a few others
	//not sure if I should add any of there, or make som of them dfault adapted MOD_ACID MOD_GAS MOD_IMPACT MOD_GAS_BLOCKABLE MOD_DROWN MOD_POISON MOD_ELECTRICWATER MOD_ELECTRIC MOD_LAVA MOD_SLIME       
	_AddMODToCannotAdaptList( MOD_MELEE ); //batleth, (hazard)teammates, 
...
float BasicArmor::ResolveDamage( float damage , int meansOfDeath , const Vector &direction , const Vector &position , Entity *attacker )
{
	...
	if ( ( meansOfDeath == MOD_ARMOR_PIERCING ) || ( meansOfDeath == MOD_IMOD_PRIMARY ) ||
		...
		//hzm gamefix daggolin - armor doesn't protect you from drowning anymore...
		( meansOfDeath == MOD_DROWN ) ||
		//hzm gamefix chrissstrahl - armor doesn't protect you from poisoning and stasis anymore...
		( meansOfDeath == MOD_GAS ) || ( meansOfDeath == MOD_POISON ) || ( meansOfDeath == MOD_STASIS ) ||
		//hzm coop mod chrissstrahl - if a trigger hurt or anything heals player (negative damage) return directly...
		( damage < 0 )
		){return damage;}
...
//HZM coop mod chrissstrahl - use singeplayer values here
	...
	//HZM coop mod chrissstrahl - armor absorbs all the damage, just like in stvef1 ?
	if ( game.coop_isActive ){
		//HZM coop mod chrissstrahl - absorb most falling damage
...
{
	//hzm coop mod chrissstrahl, we don't want the armor to vanish during coop
	if ( game.coop_isActive ){
		return;
	}
