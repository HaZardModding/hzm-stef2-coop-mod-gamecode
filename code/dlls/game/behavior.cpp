...
BehaviorReturnCode_t	FlyCloseToPlayer::Evaluate( Actor &self )
{
	Vector goal = Vector( 0, 0, 0 ); //[b607] chrissstrahl - fixed bad code vector declaration - thanks to daggolin
	Player *player = NULL;

	//hzm gamefix chrissstrahl - make this work in multiplayer
	if ( g_gametype->integer == GT_SINGLE_PLAYER ){
		player = (Player *)g_entities[0].entity;
	}else{
		bool bGetClosest = true;
		Entity *currentEnemy;
		currentEnemy = self.enemyManager->GetCurrentEnemy();
		...
		if (currentNode->thread != "" )
			{
				if ( !ExecuteThread( currentNode->thread ) )
				{
					...
					if ( coop_serverError( va( "FlySplinePath could not run thread '%s' from info_splinepath '%s'\n" , currentNode->thread.c_str() , currentNode->targetname.c_str()) , true ) ) {
   ...
   if (offsetOrigin)
	...
	if ( ( stuck > 1 ) || ( avoidtime <= level.time ) )
      ...
      ang = dir.toAngles();
		...
		/* [b607] chrissstrahl - again dead code
		if ( ( length > 150.0f ) && random_allowed && !use_temp_goal )
			{
			//ang[YAW]   += G_Random( 20 ) - 5.0;
			//ang[PITCH] += G_Random( 20 ) - 5.0;
