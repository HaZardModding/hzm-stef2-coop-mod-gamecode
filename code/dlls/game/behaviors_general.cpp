...
//hzm coop mod chrissstrahl - we need to access the multiplayerManager
#include "mp_manager.hpp"

extern Player* coop_returnPlayerClosestTo( Entity *eMe );
extern Player* coop_returnPlayerFavored( void );
extern Player* coop_returnPlayerInCallvolumeClosestTo( Entity *eMe );
...
void GotoSpecified::SetArgs ( Event *ev)
{
	...
	//hzm coop mod chrissstrahl - how I hate these fucking asserts, die the commentary death, bitch!
	//////////////////////////////////////////////////////DIE!/////////////////////////////assert ( _state != GOTO_SPEC_FAILED );
...
void MoveFromConeOfFire::_setDirectionVectors(Actor &self)
   {
	...
	//hzm coop mod chrissstrahl - manage sp and mp differently
	if ( g_gametype->integer == GT_SINGLE_PLAYER ){
		player = GetPlayer( 0 );
	}
	else{
	//hzm coop mod chrissstrahl - get the best closest player
		player = coop_returnPlayerClosestTo( ( Entity * )&self );
	//hzm coop mod chrissstrahl - make sure that player is not in spectator mode, it could cause stange acting of the ACTOR
		if ( multiplayerManager.isPlayerSpectator( player ) ){
			return;
		}
	}
	//hzm gameupdate, abort if there is no player
	if ( !player ){
		return;
	}
...
{
//hzm coop mod chrissstrahl - get closest player in coop
...
//hzm gameupdate chrissstrahl - get closet player to this ACTOR
	if ( !self.followTarget.specifiedFollowTarget ){
		Player* player = NULL;
		player = coop_returnPlayerClosestTo((Entity*)&self);
		if ( ! player ){
...
{
	//hzm gameupdate chrissstrahl - this has its issues in multiplayer when a player leaves
	//so we add a check if the player really exists: self.followTarget.specifiedFollowTarget
	//or it will crash the game in the following fucntions, on a inline vector

	// FindFollowTarget is expensive, we're putting a timer on it to keep it throttled
	//hzm coop mod chrissstrahl -  added check
...
BehaviorReturnCode_t GroupFollow::Hold( Actor &self )
{
	...
	//hzm gameupdate chrissstrahl - this also does not well in mp if a player is leaving while a ai want to go to the player
	if ( !self.followTarget.currentFollowTarget ){
		return BEHAVIOR_EVALUATING;
	}
...
BehaviorReturnCode_t GroupFollow::CloseWithTarget( Actor &self )
{
	...
	*/
	...
	//hzm gameupdate chrissstrahl - this will not work if a player leaves the game while teh ai wants to follow
	if ( !self.followTarget.currentFollowTarget ) {
		return BEHAVIOR_EVALUATING;
	}
	//hzm eof
...
float GroupFollow::ComputeAnimationRate( Actor &self, const str &animationName, const float scale )
{
	...
	//hzm gameupdate chrissstrahl - make this work in multiplayer, it crashes the game sometimes
	//added check if self.followTarget.currentFollowTarget exists
	//!self.followTarget.specifiedFollowTarget - this might also have worked
	if ( !fSmallEnough( myNewAnimationSpeed, fEpsilon() ) && self.followTarget.currentFollowTarget )
	{
...
void GotoLiftPosition::_findLiftNode( Actor &self )
	...
	//hzm coop mod chrissstrahl - it seams we don't need a specific player here, so return any player (alive, not spec are favoured)
...
{
	Player* player = NULL;
	Entity *eActorEntity = &self;

	//hzm coop mod chrissstrahl - make sure it works in sp regardles of the coop md additons
	if ( g_gametype->integer == GT_SINGLE_PLAYER ){
		player = GetPlayer( 0 );
	}
	//hzm coop mod chrissstrahl - added support for multiplayer
	else{
		//Entity* epActor = &eActorEntity;
		player = coop_returnPlayerInCallvolumeClosestTo( eActorEntity );
	}

	//hzm coop mod chrissstrahl - prevent the game from chrashing if the given player does not exist
	if ( !player )
		return;
