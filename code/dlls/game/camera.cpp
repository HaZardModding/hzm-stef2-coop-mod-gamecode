	...
	if ( node )
	{
		...
		if ( node->thread != "" )
		{
			if ( !ExecuteThread( node->thread ) )
			{
				...
				if ( coop_serverError( va( "Camera could not start thread '%s' from info_splinepath '%s'\n" , node->thread.c_str() , node->targetname.c_str() ) , true ) ) {
...
void Camera::StartMoving( Event* ev )
{
	...
	{
		//hzm coop mod chrissstrahl - quit complete game, allow reboot if server is dedicated
		if ( coop_serverError( va( "Camera '%s' with START_ON selected, but no target specified." , TargetName() ) , true ) ) {
	...
	if ( !targetEnt )
	{
		...
		{
			//hzm coop mod chrissstrahl - quit complete game, allow reboot if server is dedicated
			if ( coop_serverError( va( "Can't find target '%s' for camera\n" , Target()) , true ) ) {
...
void Camera::FollowEvent( Event* ev )
{
	...
	{
		//[b60021] chrissstrahl - have additional data print out
...
void Camera::Watch( const str& watch, float time )
{
	...
	//[b60011] chrissstrahl - I think we don't need this - we want this to execute without being in a cinematic for everyone
...
void Camera::SetFOV( float fov, float time )
{
	...
	else
	{
		...
		//hzm gameupdate chrissstrahl - updated for multiplayer use
		gentity_t *gentity;
		...
		{
			//hzm gameupdate chrissstrahl - get the fov from any player
			int i;
...
{
	//hzm gamefix chrissstrahl - grab any player in multiplayer
...
void CameraManager::Load( Event* ev )
{
	...
	//hzm coop mod chrissstrahl - try loading a coop version of the file, if it exists and we are in coop, otherwise load default
	filename = coop_serverModifiedFile( va( "cams/%s.cam" , name.c_str() ) );
