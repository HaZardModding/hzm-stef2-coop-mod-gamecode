...
bool coop_actorDeadBodiesHandle(Entity* actor)
{
	...
	if (game.coop_deadBodiesPerArea > 0 && !coop_checkDeadBodiesStayDisabled()) { //[b60021] chrissstrahl - added support to disallow dead bodies for a level
		Event *rename_event;
	...
	//[b60021] chrissstrahl - added support to disallow dead bodies for a level
	if (!coop_checkDeadBodiesStayDisabled()) {

		//[b60021] chrissstrahl - stop electrified effect on dead body - moved here
		Event* newEvent;
		newEvent = new Event(EV_ClearCustomShader);
		newEvent->AddString("electriclines");
		Entity* eAct = (Entity*)actor;
	...
	//return info that this body will not stay //[b60021] chrissstrahl - added level floatvar check for coop_disableDeadBodies
	if (game.coop_deadBodiesPerArea < 1 || coop_checkDeadBodiesStayDisabled()) {
		return false;
	}
