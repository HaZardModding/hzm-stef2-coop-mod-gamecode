	...
	if (stricmp(command.c_str(), "map") == 0 || stricmp(command.c_str(), "nextmap") == 0)
	{
		...
		if (coop_returnCvarInteger("coop_votedisable") == 1) {
			if (strnicmp(sMapRealName.c_str(), "coop_",5) != 0 && //maps starting with coop_
				...
				coop_parserIsItemInCategory("coop_mod/ini/maplist.ini", sMapRealName.c_str(),"singlePlayerMission") != true &&
				coop_parserIsItemInCategory("coop_mod/ini/maplist.ini", sMapRealName.c_str(),"singlePlayerIgm") != true &&
				coop_parserIsItemInCategory("coop_mod/ini/maplist.ini", sMapRealName.c_str(),"singlePlayerSecret") != true &&
				coop_parserIsItemInCategory("coop_mod/ini/maplist.ini", sMapRealName.c_str(),"coopIncluded") != true
			) {
				if (coop_checkPlayerLanguageGerman(player)) {
...
int coop_vote_mapNxtPrevValidate(Player* player, const str &command, const str &arg, str &_voteString)
{
	...
	coop_parserGetItemsFromCategory("coop_mod/ini/vote_maplist.ini", startCategories, endCategories, "categories");

	if (!startCategories) {
		if (coop_checkPlayerLanguageGerman(player)) {
			player->hudPrint("FEHLER: coop_mod/ini/vote_maplist.ini sektion: categories, besch. oder fehlt.\n");
		}
		else {
			player->hudPrint("ERROR: coop_mod/ini/vote_maplist.ini section: categories, is damaged or missing.\n");
		}
		gi.Printf(va("MultiplayerManager::callVote (%s) ERROR: coop_mod/ini/vote_maplist.ini section: categories, is damaged or missing.\n", command.c_str()));
	...
	while (currentCategories) {
		...
		coop_parserGetItemsFromCategory("coop_mod/ini/vote_maplist.ini", start, end, currentCategories->value.c_str());


		//abbort on error///////////////////////////////////////////////////////
		if (!start) {
			if (coop_checkPlayerLanguageGerman(player)) {
				player->hudPrint(va("FEHLER: coop_mod/ini/vote_maplist.ini sektion: %s, kaputt oder fehlt.\n", currentCategories->value.c_str()));
			}
			else {
				player->hudPrint(va("ERROR: coop_mod/ini/vote_maplist.ini section: %s, is damaged or missing.\n", currentCategories->value.c_str()));
			}
			gi.Printf(va("MultiplayerManager::callVote (%s) ERROR: coop_mod/ini/vote_maplist.ini section: %s, is damaged or missing.\n", command.c_str(), currentCategories->value.c_str()));
	...
	if (stricmp(sMapToLoad.c_str(), "") == 0) {
		if (coop_checkPlayerLanguageGerman(player)) {
			player->hudPrint(va("%s ist nicht aufgelistet in: coop_mod/ini/vote_maplist.ini.\n", sMapRealName.c_str()));
		}
		else {
			player->hudPrint(va("%s is not listed in: coop_mod/ini/vote_maplist.ini.\n", sMapRealName.c_str()));
		}
		gi.Printf(va("%s is not listed in: coop_mod/ini/vote_maplist.ini.\n", sMapRealName.c_str()));
		return 1;
	}
...
bool coop_vote_lastmanstandingSet(const str _voteString)
{
	...
	coop_parserIniSet("coop_status.ini", "lastmanstanding", (bool)iStatus, "server");
...
bool coop_vote_respawntimeSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	coop_parserIniSet("coop_status.ini", "respawntime", (int)multiplayerManager.getRespawnTime(), "server");
...
bool coop_vote_airaccelerateSet(const str _voteString)
{
	...
	coop_parserIniSet("coop_status.ini", "airaccelerate", iVal , "server");
...
bool coop_vote_stasistimeSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	coop_parserIniSet("coop_status.ini", "stasistime", game.coop_stasisTime, "server");
...
bool coop_vote_awardsSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	coop_parserIniSet("coop_status.ini", "awards", sValue, "server");
...
bool coop_vote_friendlyfireSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	coop_parserIniSet("coop_status.ini", "friendlyFire", sMultiplier, "server");
...
bool coop_vote_maxspeedSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	coop_parserIniSet("coop_status.ini", "maxSpeed", game.coop_maxspeed, "server");
...
bool coop_vote_teamiconSet(const str _voteString)
{
	...
	//hzm coop mod chrissstrahl - save changes directly to ini
	icon = (bool)atoi(icon);
	coop_parserIniSet("coop_status.ini", "teamIcon", icon.c_str(), "server");
...
bool coop_vote_deadbodiesSet(const str _voteString)
{
	...
	//save to ini
	icon = atoi(icon);
	coop_parserIniSet("coop_status.ini", "deadbodies", icon.c_str(), "server");
