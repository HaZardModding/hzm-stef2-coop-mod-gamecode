...
(
	...
	NULL ,
	NULL ,
	"Event to set a idle animation on a entity."
);
//end of hzm
...
Event EV_EntitySVFlags
   ...
   "broadcast - always send this entity to the client"
	//[b607] chrissstrahl added more usefull commands
	"noclient - don't send entity to clients, even if it has effects"
	"broadcast - send to all connected clients"
	"monsterclip - treat as CONTENTS_MONSTERCLIP for collision"
	"playerclip - treat as CONTENTS_PLAYERCLIP for collision"
	"sendonce - Send this entity over the network at least one time"
	"use_current_origin - entity->currentOrigin instead of entity->s.origin"
   );
...
//
//--------------------------------------------------------------
//hzm gameupdate chrissstrahl - allows to set idle animation by event
	void Entity::SetIdleAnimation( Event *ev )
	{
		Event *evAnim = new Event( EV_Anim );
		evAnim->AddString( "idle" );
		this->PostEvent( evAnim , 0.0f );
	}
...
Entity::~Entity()
{
	...
	//hzm coop mod chrissstrahl - remove objectives marker
	coop_objectivesMarkerRemove( this );
...
void Entity::setModel( const char *mdl )
{
	...
	coop_serverMonitorTikiCache( model );
...
void Entity::setViewModel
   ...
   if ( LoadingSavegame && ( this == world ) )
	...
	//hzm coop mod chrissstrahl - test
	//we want to find out how the loading of models and the tiki cache are related
	coop_serverMonitorTikiCache( (str)mdl );
...
void Entity::BecomeSolid
	...
	if ( ( model.length() ) && ( ( model[ 0 ] == '*' ) || ( strstr( model.c_str(), ".bsp" ) ) ) )
		...
		{
		//hzm coop mod chrissstrahl - set state so that the think thread will handle it right
		if (g_gametype->integer > GT_SINGLE_PLAYER) { //[608] chrissstrahl - added check to prevent player staying notsolid in sp
...
void Entity::BecomeNonSolid
	...
	//hzm coop mod chrissstrahl - make sure we don't make someone solid who shoudn't be
	if ( isSubclassOf( Actor ) || isSubclassOf( Player ) ){
		_makeSolidASAP = false;
		_makeSolidASAPTime = 0.0f;
	}
...
void Entity::SVFlags
   ...
   for ( i = 1; i <= ev->NumArgs(); i++ )
      ...
      if ( !stricmp( flag, "broadcast" ) )
         ...
         mask = SVF_SENDONCE;
	//[b607] chrissstrahl - add the missing usefull flags
	  else if (!stricmp(flag, "use_current_origin"))
		  mask = SVF_USE_CURRENT_ORIGIN;
	  else if (!stricmp(flag, "sendonce"))
		  mask = SVF_SENDONCE;
	  else if (!stricmp(flag, "monsterclip"))
		  mask = SVF_MONSTERCLIP;
	  else if (!stricmp(flag, "playerclip"))
		  mask = SVF_PLAYERCLIP;
	  else if (!stricmp(flag, "noclient"))
		  mask = SVF_NOCLIENT;
	//[b607] end
      else
         {
...
void Entity::CreateEarthquake( Event *ev )
   {
	...
	//if ( origin == vec_zero )
	//{
		//hzm coop mod chrissstrahl - no one really cares... this is just spam
...
void Entity::SetUserVar1
   ...
   Event *ev
...

	{
		...
		//hzm coop mod chrissstrahl - this is a dirty little hack to grab the uservar1 when it is set, didn't find any other way to do this
		if ( this->targetname == "world" ){
			game.coop_author = ev->GetString( 1 );
			coop_objectivesUpdateUservar(1);
		}
	}
...
void Entity::SetUserVar2
   ...
   Event *ev
...

	{
		...
		//hzm coop mod chrissstrahl - this is a dirty little hack to grab the uservar2 when it is set, didn't find any other way to do this
		if ( this->targetname == "world" ){
			game.coop_story = ev->GetString( 1 );
			coop_objectivesUpdateUservar(2);
		}
	}
...
void Entity::SetUserVar3
   ...
   Event *ev
...

	{
		...
		//hzm coop mod chrissstrahl - this is a dirty little hack to grab the uservar3 when it is set, didn't find any other way to do this
		if ( this->targetname == "world" ){
			game.coop_story_deu = ev->GetString( 1 );
			coop_objectivesUpdateUservar(3);
		}
	}
...
void Entity::setMissionObjective(Event* ev)
{
	...
	//hzm coop mod chrisssstrahl - do not spawn indicator twice
	bool balreadySet = false;
	if ( _missionObjective ){
		balreadySet = true;
	}
	...
	if ( edict->s.missionObjective  ){
		//hzm coop mod chrisssstrahl - spawn objective marker at the location of this entity, but only once
		if ( !balreadySet )coop_objectivesMarker( this );
		//hzm eof
	...
	{
		//hzm coop mod chrisssstrahl - remove objective marker
		coop_objectivesMarkerRemove( this );
		//hzm eof
...
void Entity::isWithinDistanceOf(Event* ev)
{
	...
	if (!destination) {
		gi.Printf(va("isWithinDistanceOf failed for Actor $%s, the Entity given in parameter1 does not exist!\nRemember, you can not use Pathnodes and Helpernodes\n",this->targetname.c_str()));
	}
...
extern void G_Physics_Toss( Entity *ent );
void Entity::BubbleThink( void )
{ //FIXME - Keep this in Entity or create a new class?
	if ( !stricmp(targetname, "breathBubbleSplash") )
	{ // Splash when leaving the water
		if ( level.time < bubbleNextThink ) return;
		bubbleNextThink = level.time + 0.04f;

		if ( edict->s.scale < bubbleSplashMax )
		{
			setScale(edict->s.scale + 0.005f);
		}
		else
		{
	...
	// Increase the bubble's velocity on its way up...
	if ( bubbleNextSpeedIncrease < level.time && bubbleTotalSpeedIncrease < 200 )
	{
		velocity[2] += 25;
		bubbleTotalSpeedIncrease += 25;
		bubbleNextSpeedIncrease += .5f;
	}
	...
	if ( !waterlevel || !(watertype & CONTENTS_WATER) )
	{ // Not in water anymore!
		abort = true;
	}
	
	// Check for solid things we might have encountered...
	trace_t	trace;
	trace = G_Trace( origin, mins, maxs, (origin + (velocity * level.frametime)), this, MASK_SOLID, false, "G_PushEntity" );
	if ( trace.fraction != 1.0f || ( trace.startsolid && trace.ent && trace.ent->entity != world ) )
	{ // Encountered something (wall?)
		abort = true;
	}

	if ( bubbleMaxLife < level.time )
	...
	{ // Remove
		...
		// Spawn the splash!
		ent = SpawnEffect( "rainsplash.spr", origin, Vector(90, 0, 0), 0.0f );
		ent->SetTargetName( "breathBubbleSplash" );
		ent->setScale(edict->s.scale / 4);
		ent->bubbleSplashMax = edict->s.scale / 2;

		// Play the sound!
		ent = SpawnSound( "sound/environment/water/drip.wav", origin, 0.7f, 1.0f );
		...
		for ( i = 0; i < 10; i++ )
		{
			...
			if ( waterlevel || (watertype & CONTENTS_WATER) ) break;
		}
	}
}
