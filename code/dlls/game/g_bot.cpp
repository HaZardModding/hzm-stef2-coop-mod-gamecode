...
static void G_AddBot( char *name, float skill, const char *team, const char *specialty, char *altname) {
	...
	Info_SetValueForKey( userinfo, "rate", "15000" ); //[b607] chrissstrahl - bots don't need that rate really //was 25000
	//[b607] chrissstrahl - reduce networking further for bots, they don't need to see where player is going (extra animation set)
	Info_SetValueForKey( userinfo, "mp_lowBandwidth", "1"); 
	//[b607] end
...
//	bot->inuse = qtrue;
	...
	if ( G_ClientConnect( clientNum, qtrue, qtrue, qfalse ) ) { //[b607] daggo - don't check passwords for bots (even if the right one is set)
		return;
	}
...
void SV_AddBot_f( void ) {
	...
	if ( !gi.Cvar_VariableIntegerValue( "bot_enable" ) ) {
		gi.Printf("Can't add bot cvar bot_enable must be set to 1\n");
		return;
	}
