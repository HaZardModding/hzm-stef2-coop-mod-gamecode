...
extern "C" void G_InitGame( int startTime, int randomSeed )
{
	...
	try
	{
		...
		if (coop_returnBool(coop_parserIniGet("coop_status.ini", "errorboot", "server")))
		{
			str sErrorMap = coop_parserIniGet("coop_status.ini", "errormap", "server");
			...
			//in a reboot loop
			if (!coop_returnBool(coop_parserIniGet("coop_status.ini", "errorfatal", "server"))) {
				sStartMap = sErrorMap;
			}

			coop_parserIniSet("coop_status.ini", "errorboot", "false", "server");
			str sErrorMsg = coop_parserIniGet("coop_status.ini", "errortext", "server");
			...
			bool bServerHasRebooted = coop_returnBool(coop_parserIniGet("coop_status.ini", "rebooting", "server"));
			//sStartMap = coop_phraserIniGet( "coop_status.ini" , "startmap" , "server" );
				...
				)
			{
				...
				sStartMap = coop_parserIniGet("coop_status.ini", "startmap", "server");
...
void G_AllocGameData( void )
{
	...
	//hzm gameupdate daggolin - delayed server commands
	for ( i = 0; i < MAX_CLIENTS; i++ )
	{
		pendingServerCommandList[i] = NULL;
	}
...
void G_BeginIntermission( const char *map )
{
	...
	// Display scores for all the clients
	//hzm coop mod chrissstrahl - don't display score during coop
...
void G_ExitLevel( void )
{
	...
	// kill the sounds
	//[b608] chrissstrahl - do execute this if it is not a dedicated server
	if (g_gametype->integer == GT_SINGLE_PLAYER || dedicated->integer != 0) {
...
extern "C" void G_RunFrame( int levelTime, int frameTime )
{
	...
	try
	{
		...
		for( edict = active_edicts.next, num = 0; edict != &active_edicts; edict = level.next_edict, num++ )
		{
			...
			//hzm coop mod - Little hack to detect actors getting instructions to move at a very high rate due to loops in scripts
			if ( ent->isSubclassOf( Actor ) )
				((Actor*)ent)->wakeUpCount = 0;
		}
...
//hzm coop mod chrissstrahl - this makes entities appear on the player his radar that are normaly in a different area or to far away to be send to the player 
...
extern "C" void G_ClientBegin( gentity_t *ent, const usercmd_t *cmd )
{
	try
	{
		...
		else
		{
			...
			level.spawn_bot = (ent->svflags & SVF_BOT); //[b607] daggolin - get flag and set it on level so it can be reset on edict else where
			player = new Player;
		}
...
qboolean G_duplicateName( const char *name )
{
	...
	//hzm gamefix daggolin - clean the name
	str nameClean;
	...
	for ( i = 0; i < game.maxclients; i++ )
	{
		...
		//hzm gamefix daggolin - clean the other name, too
		str netnameClean;
		for ( i = 0; i < strlen( edict->client->pers.netname ); i++ )
		{
			if ( ( edict->client->pers.netname[i] == '^' ) && ( edict->client->pers.netname[i + 1] >= '0' ) && ( edict->client->pers.netname[i + 1] <= '9' ) )
			{
				i++;
				continue;
			}
			...
			{
				netnameClean += (char)tolower(edict->client->pers.netname[i]);
			}
		}

		//hzm gamefix daggolin - prevent duplicate names...
		if ( strcmp( /*edict->client->pers.netname, name*/netnameClean, nameClean ) == 0 )
		{
			return true;
		}
...
//hzm gamefix daggolin - count same names
//[b607] chrissstrahl - fixed bad code creating a infinity loop if more than 7 players/bots where on the server
qboolean IsUniqueName(gentity_t *ent, const char *name)
{
	gentity_t	*edict;
	str			 nameClean = "";
	...
	for (int i = 0; i < game.maxclients; i++) {
		edict = &g_entities[i];
		if (!edict->inuse || !edict->entity || !edict->client || edict == ent) {
			continue;
		}

		str netnameClean = "";
		for (int i2 = 0; i2 < strlen(edict->client->pers.netname); i2++) {
			if ((edict->client->pers.netname[i2] == '^') && (edict->client->pers.netname[i2 + 1] >= '0') && (edict->client->pers.netname[i2 + 1] <= '9')) {
				i2++;
				continue;
			}else {
				netnameClean += (char)tolower(edict->client->pers.netname[i2]);
			}
		}
		if (strcmp(netnameClean, nameClean) == 0) {
			return false;
		}
	}
	return true;
...
//hzm gamefix daggolin - count same names
void BuildUniqueName( gentity_t *ent, const char *newName, char *name )
{
	char intendedName[MAX_NETNAME];
	char builtName[ MAX_NETNAME ];
	char suffix[3];

	int i;

	// Make sure we have enough space for the number and the ending ^8 (#^8)
	memset( intendedName, 0, sizeof(intendedName) );
	strncpy( intendedName, newName, (MAX_NETNAME - 1 - 3) );

	if ( strstr(intendedName, "^") ) Q_strncpyz( suffix, "^8", sizeof(suffix) );
	else							 Q_strncpyz( suffix, "", sizeof(suffix) );

	// If we already have ^8 at the end remove it...
	if ( strlen(suffix) && intendedName[strlen(intendedName)-2] == '^' && intendedName[strlen(intendedName)-1] == '8' ) intendedName[strlen(intendedName)-2] = 0;

	if ( IsUniqueName( ent, intendedName ) )
	{
		Q_strncpyz( builtName, va("%s%s", intendedName, suffix), sizeof(builtName) );
		strcpy( name, builtName );
		return;
	}

	for ( i = 2 ; i <= 9 ; i++ )
	{ //Give up after 9, because we only have 4 characters reserved behind the name( NAME#^8 )
		Q_strncpyz( builtName, va("%s%d%s", intendedName, i, suffix), sizeof(builtName) );

		if ( IsUniqueName( ent, builtName ) )
		{
			strcpy( name, builtName );
			return;
		}
...
extern "C" void G_ClientUserinfoChanged( gentity_t *ent, const char *userinfo )
{
	...
	try
	{
		...
		*/
		...
		//hzm gamefix daggolin - make sure tempName only contains 0s
		...
		//hzm gamefix chrissstrahl - cleanup the player name
		str tempCleanName = "";
		...
		for ( i = 0; i < strlen( tempName ); i++ )
		{
			...
			{
				tempCleanName += (char)tolower(tempName[i]);
			}

			validName = true;
		}

		//hzm coop mod chrissstrahl - disallow the word Coop in a name, because players should not fake coop mod messages
		if ( game.coop_isActive )
		{
			if ( game.coop_isActive )
			{
				if ( strstr( tempCleanName , "coop" ) )
				{
					validName = false;
				}
			}
		...
		//hzm gamefix chrissstrahl - disallow the word Server in the player name, because the player should not fake the server messages
		if ( strstr( tempCleanName, "server" ) )
		{
			validName = false;
		}
		...
		if ( strcmp( ent->client->pers.netname, tempName ) != 0 )
		{
			...
			BuildUniqueName( ent, tempName, newName );
		...
		/* //hzm gamefix daggolin - Handled in "BuildUniqueName" now
		...
		*/
		...
		//hzm coop mod chrissstrahl - we want to force a different bsp location for the player, so that the server can load modified BSP
		//gi.setConfigstring( ? , ? );
...
extern "C" const char *G_ClientConnect( int clientNum, qboolean firstTime, qboolean isBot, qboolean checkPassword )
{
	...
	try
	{
		...
		if ( SV_FilterPacket( value ) )
		{
			...
			multiplayerManager.HUDPrintAllClients( va( "%s ^3faild to connect: $$BannedIP$$\n" , Info_ValueForKey( userinfo , "name" ) ) );
			gi.Printf( "%s ^8faild to connect: $$BannedIP$$\n" , Info_ValueForKey( userinfo , "name" ) );
			return "$$BannedIP$$";
		}
...
//hzm gameupdate chrissstrahl - allow player to enter when server has no password, but player has a password set
			str serverPassword = password->string;
			if ( serverPassword.length() > 0 && strcmp( password->string , value ) != 0 )
			{
				//hzm gameupdate chrissstrahl -  Inform all of the players that a player failed to connect
		...
		//if ( coop_reboot->integer == 1 ){
			...
			//return "Server Rebooting, please try 'reconnect' in a few seconds.";
		//}
...
{
	//[b607] chrissstrahl - make multi compatible
	Player * player = coop_returnPlayerNotFavored();
