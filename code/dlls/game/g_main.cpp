...
extern "C" void G_InitGame( int startTime, int randomSeed )
{
	...
	try
	{
		...
		if (coop_returnBool(coop_parserIniGet("ini/server.ini", "errorboot", "server")))
		{
			str sErrorMap = coop_parserIniGet("ini/server.ini", "errormap", "server");
			...
			//in a reboot loop
			if (!coop_returnBool(coop_parserIniGet("ini/server.ini", "errorfatal", "server"))) {
				sStartMap = sErrorMap;
			}

			coop_parserIniSet("ini/server.ini", "errorboot", "false", "server");
			str sErrorMsg = coop_parserIniGet("ini/server.ini", "errortext", "server");
			...
			bool bServerHasRebooted = coop_returnBool(coop_parserIniGet("ini/server.ini", "rebooting", "server"));
			//sStartMap = coop_phraserIniGet( "ini/server.ini" , "startmap" , "server" );
				...
				)
			{
				...
				sStartMap = coop_parserIniGet("ini/server.ini", "startmap", "server");
