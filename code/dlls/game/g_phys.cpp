...
//hzm gameupdate daggolin - called each frame for every entity
void gamefix_G_RunEntity( Entity *ent )
{
	if ( !ent || !ent->edict || !ent->edict->inuse || ent->client || !ent->targetname.c_str() )
		return;

	const char *string = ent->targetname.c_str();

	if ( string == (const char*)0x00000001 ) return; //hzm FIXME: 0x00000001?!? What're we doing wrong that we get a targetname == 0x00000001 case on dm_pool?

	if ( strlen(string) && ((!stricmp( string, "breathBubble" ) || !stricmp( string, "breathBubbleSplash" ))) )
	{
		ent->BubbleThink();
	}
}
...
void G_RunEntity( Entity *ent )
{
	...
	//hzm gameupdate - called each frame for every entity
	gamefix_G_RunEntity( ent );
