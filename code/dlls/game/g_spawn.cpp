...
Entity *SpawnArgs::Spawn( void )
{
	...
	//[b607] chrissstrahl - prevent spawning of bad models (Player class/dm_munro.tik)
	if (cls != NULL && !Q_stricmp(cls->classname,"Player")) {
		gi.Printf("Can't be spawned! This is using classname: %s!\n", cls->classname);
		if (g_gametype->integer == 1) {
			multiplayerManager.HUDPrintAllClients(va("Can't be spawned! This is using classname: %s!\n", cls->classname));
		}
		return NULL;
	}
