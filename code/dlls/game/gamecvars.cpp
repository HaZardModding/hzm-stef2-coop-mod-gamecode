...
// New server cvars
//hzm coop mod chrissstrahl - add gamevars for the cvars, so we can access it in the gamecode, it is in the engine but not in the gamecode
...
void CVAR_Init( void )
{
	...
	sv_maxbots = gi.cvar("sv_maxbots", "0", 0);
	//[b607] chrissstrahl - fail server after some time if empty
	coop_autoFailtime = gi.cvar("coop_autofailtime", "30", 0);


	//hzm coop mod chrissstrahl - we want sv flood protect to be avilable
	sv_floodprotect = gi.cvar( "sv_floodprotect" , "1" , CVAR_ARCHIVE );

	//hzm coop mod chrissstrahl changed from: CVAR_LATCH
	skill	= gi.cvar( "skill",	"1", CVAR_SERVERINFO );

	//hzm coop mod chrissstrahl changed from: CVAR_SERVERINFO
	nomonsters = gi.cvar( "nomonsters" , "0" , CVAR_ARCHIVE );

	developer						= gi.cvar( "developer",						"0",		CVAR_CHEAT );
	precache						= gi.cvar( "sv_precache",					"1",		0 );
#ifdef __linux__
	g_gametype						= gi.cvar( "g_gametype" ,					"1" ,		CVAR_SERVERINFO | CVAR_LATCH );
	dedicated						= gi.cvar( "dedicated",						"2",		CVAR_LATCH );
#else
...
#else
	//hzm gameupdate, start listen server with 4 player slots, if multplayer and not set
	...
	sv_showdamagecolors				= gi.cvar( "sv_showdamagecolors",			"0",		0 );
	...
	sv_fps							= gi.cvar( "sv_fps",						"30",		CVAR_SERVERINFO );
	...
	sv_noclipspeed					= gi.cvar( "sv_noclipspeed",				"200",		0 );
