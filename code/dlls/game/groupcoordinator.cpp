...
void Group::RunGroupDeathThread(Entity *entity)
{
	...
	CThread * thread;
	thread = ExecuteThread(_groupDeathThread, true, entity );

	if ( !thread )
	{
		Entity* eActivate = coop_returnEntity( _groupDeathThread.c_str() );
		if ( !eActivate )
		{
			gi.Printf( "Group::RunGroupDeathThread - Could not find thread/entity (%s) !\n" , _groupDeathThread.c_str() );
		}
		else
		{
			...
			{
				Actor *act = ( Actor* )entity;
				act->lastAttacker != NULL ? event->AddEntity( act->lastAttacker ) : event->AddEntity( world );
			}
			else
			{
...
void GroupCoordinator::RemoveEntityFromGroup( Entity *ent , int ID )
{
	...
	for ( i = 1; i <= _groupList.NumObjects() ; i++ ){
		...
		if ( group->GetGroupID() == ID ){
			group->RemoveMember( ent );
			//[b607]chrissstrahl - fix bug that if one group is empty it won't clear until all entites in this group are removed from the level
			if (group->CountMembers() == 0) {
				//gi.Printf("HZM-Debug ::RemoveEntityFromGroup (%i) last Member, removing group\n",ID);
				RemoveGroup(ID);
			}
		}
	}
...
void GroupCoordinator::RemoveGroup( int ID )
{
	...
	for ( int i = _groupList.NumObjects(); i > 0  ; i-- ){
		...
		if ( group->GetGroupID() == ID ){
			//[b607]chrissstrahl - fix bug that if one group is removed it won't clear it (containers retain data)
			//gi.Printf("HZM-Debug ::RemoveGroup (%i) ClearList\n", ID);
			//group->ClearList();
			//gi.Printf("HZM-Debug ::RemoveGroup (%i) RemoveObjectAt (%i)\n", ID, i);

			_groupList.RemoveObjectAt( i );
			
			//[b607]chrissstrahl - fix bug that if one group is removed it won't clear it (containers retain data)
			//gi.Printf("HZM-Debug ::RemoveGroup (%i) delete group\n", ID, i);
