...
void Interpreter::RunError( const char *error, ... )
{
	...
	if ( coop_serverError( "Program error" , true ) ) {
...
void Interpreter::ThreadCall( const Interpreter *source, dfunction_t *newf, int args )
{
	...
	//hzm coop test chrissstrahl - try how we can implement objectives
	int parameterType[16]; //16 is the maximum of paramaters a script function can handle
	str parameterString[16];
	float parameterFloat[16];
	//Vector parameterVector[16];
	...
	for( i = 0; i < args; i++ )
	{
		...
		//hzm coop mod chrissstrahl - test how to implement objectives
			parameterString[i] = program->strings[localstack[i]].s;
			parameterType[i] = 1;
		...
		case ev_vector :
			...
			//hzm coop mod chrissstrahl - test how to implement objectives
			/*parameterVector[i] = Vector( program->localstack_type[ i + 0 ] ,
								program->localstack_type[ i + 1 ] ,
								program->localstack_type[i + 2] );*/
			parameterType[i] = 2;
		...
		//hzm coop mod test chrissstrahl ----
			localstack_type[i] = ev_float;

			parameterFloat[i] = localstack[i];
			parameterType[i] = 3;
		//default code
            break;
		}
...
int Interpreter::LeaveFunction( void )
{
	...
	{
		//hzm coop mod chrissstrahl - quit complete game, allow reboot if server is dedicated
		if ( coop_serverError( "prog stack underflow" , false ) ) {
...
void Interpreter::Execute( Event *e )
{
	...
	while( !doneProcessing && !threadDying )
	{
		...
		case OP_ADD_S:
			//[b60025] Chrissstrahl - fix game crashing if a string operation with a deleted entity is done in the scripts
		...
		case OP_DIV_F:
			...
			{
				//hzm gamefix chrissstrahl, a assert and then you fix the issue, AGAIN....
				//assert( 0 );
				c->_float = 0;
			}
			break;
