...
void Level::Init( void )
{
	...
	m_letterbox_time              = -1;
	m_letterbox_time_start        = 1.0f / 8.0f;
	m_letterbox_dir               = letterbox_out;
	//hzm gameupdate chrissstrahl - putting this right where it belongs
	m_letterbox_lastfraction = 0.0f;
	//end of hzm
...
void Level::Start( void )
{
	...
	if ( game_script.length() )
	{
		...
		//gi.Printf( "Compiling .scr files started at: %f\n" , ( float )gi.Milliseconds() / 1000 );

		program.Load( game_script );

		//hzm coop mod chrissstrahl . we don't need this
		//gi.Printf( "Compiling .scr files finsihed at: %f\n" , ( float )gi.Milliseconds() / 1000 );
...
void Level::setSkill( int value )
{
	...
	skill_level = value; //(int) floor( value ); lol what? why floor an int? - GSIO
...
void Level::Archive( Archiver &arc )
{
	...
	//hzm gameupdate chrissstrahl - putting this right where it belongs
	arc.ArchiveFloat( &m_letterbox_lastfraction );
	//end of hzm
	...
	else
	{
		...
		if ( i == game.maxentities - 2.0f )
		{
			...
			if ( i == game.maxentities - 2.0f )
			{
				...
				if ( coop_serverError( "Level::AllocEdict: no free edicts" , false ) ) {
