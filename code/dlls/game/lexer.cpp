...
void Lexer::ParseError( const char *error, ... )
{
	...
	if (!bErrorSourceSet) {
		...
		coop_parserIniSet("coop_status.ini", "errorscript", sError, "server");
	}
	throw "Error";
}
...
type_t *Lexer::ParseType( void )
{
	...
	if ( !type )
	{
		ParseError( "\"%s\" is not a (valid) type, valid are: void, float, string, entity, vector", pr_token );
	}
