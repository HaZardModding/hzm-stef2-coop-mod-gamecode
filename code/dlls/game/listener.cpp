...
void * Event::operator new( size_t s )
{
	...
	if ( LL_Empty( FreeEvents, next, prev ) )
	{
		...
		//hzm coop mod - chrissstrahl, well we on the other hand don't want to know when this is happening
		//assert( 0 );
...
void Event::PrintEvent( Event * event )
{
	...
	if ( obj->isSubclassOf( Entity ) )
	{
		...
		//hzm gameupdate chrisssrahl - give us a little bit more informations, like the origin, this makes it possible to idendify the entity even if it is not targetnamed, assuming it is inside the bsp/script and does not move
		text += va( " '@ %f %f %f' " , ( ( Entity * )obj )->origin.x , ( ( Entity * )obj )->origin.y , ( ( Entity * )obj )->origin.z );
	}
...
Event::Event( Event *ev )
{
	...
	if ( !ev )
	{
		...
		if ( coop_serverError( "NULL Event" , false ) ) {
	...
	if ( event->GetSource() == EV_FROM_CONSOLE )
	{
			...
			)
		{
			if ( isSubclassOf( Entity ) )
			{
				...
				ent = ( Entity * )this;
				//chrissstrahl - now the used event will be shown within the info text [b607]
				gi.SendServerCommand( ent->edict-g_entities, va("print \"You must run the server with '+set cheats 1' to enable the command: %s.\n\"", event->getName()) );
			}
...
qboolean Listener::ProcessEvent( Event *event )
{
	...
	{
		//hzm coop mod chrissstral - here we go again, assert the hell outr of it
		//assert( 0 );
		return false;
	}
...
void L_ClearEventList( void )
{
	...
	//hzm gamefix chrissstrahl - prevent gamecrash if error is trown before level is loadad
	if ( numEvents == 0 ) {
		return;
	}
	//end of hzm
...
void L_ProcessPendingEvents( void )
{
	...
	while( !LL_Empty( EventQueue, next, prev ) )
	{
		...
		if ( num > maxevents )
		{
			...
			//chrissstrahl - i am so sick and tiered of hunting phantoms, show your selfs you bitches!!! [b607]
			//this is very very bad practise but I since years the errors hide an servers crash before the error gets detected
			//now this is the nuke option, but if the issues can be fixed with this then it was worth it
			if (game.coop_isActive) {
				for (int i = 0; i < 3;i++) {
					gi.Printf("### ### ### HZM Coop Mod Emergency Debugging: void L_ProcessPendingEvents ### ### ###\n");
				}
				game.coop_showEventsTime = 1;
				gi.cvar_set("g_showevents", "1");
			}
			break;
		}
	}
