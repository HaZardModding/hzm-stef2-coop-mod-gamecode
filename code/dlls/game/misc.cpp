...
//hzm coop mod chrissstrahl - don't do that carp during coop
...
//hzm gameupdate chrissstrahl - make sure players do not get stuck inside each other
	other->_makeSolidASAP = true;

	/*
	//hzm coop mod chrissstrahl - if a player is inside another player, make him not solid, and activate the makesolidASAP
	if ( other->client ){
		int i;
		...
		//other->->makeSolidASAP = false;

		for ( i = 0; i < maxclients->integer; i++ ){
			otherPlayer = ( Player * )g_entities[i].entity;
			if ( otherPlayer && otherPlayer != player ){
				if ( otherPlayer->client && otherPlayer->isSubclassOf( Player ) ){
					if ( !multiplayerManager.isPlayerSpectator( player ) && coop_checkIsEntityInBoundingBox( otherPlayer , player ) == true ){
						isInsideOther = true;
					}
				}
			}
	...
	*/
	...
	if ( teleport_thread.length() && other->isSubclassOf( Player ) )
	{
		if ( !ExecuteThread( teleport_thread.c_str() ) )
			warning( "RunThread", "could not find thread in level-script" );
	}
