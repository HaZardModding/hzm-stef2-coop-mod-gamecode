...
//hzm coop mod chrissstrahl - do not precache that during coop
...
void AwardSystem::playerKilled( Player *killedPlayer, Player *attackingPlayer, Entity *inflictor, int meansOfDeath )
{
	...
	//hzm coop mod chrissstrahl - no such awards during coop
	if ( game.coop_isActive ){
		return;
	}
...
void AwardSystem::pickedupItem( Player *player, MultiplayerItemType itemType, const char *itemName )
{
	...
	for ( i = 0 ; i < _maxPlayers ; i++ )
	{
		...
		//hzm gameupdate chrissstrahl - do not pay dialog if player is dead
		//this happens if a player with a item gets killed and another player picks it up
