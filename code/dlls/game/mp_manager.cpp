...
void MultiplayerManager::score( Player *player )
{
	...
	//--------------------------------------------------------------
	// [b60014] Coop Mod chrissstrahl - do not force the scoreboard when player is dead (show only for a few sec), also show "Press fire to spawn" hud
	//--------------------------------------------------------------
...
int MultiplayerManager::upgPlayerGetKills( Player *player )
{
	assert(player);
	...
	return _multiplayerGame->upgPlayerGetKills( player );
}
...
void MultiplayerManager::callVote( Player *player , const str &command , const str &arg )
{
	...
	//--------------------------------------------------------------
	//[b60022] chrissstrahl - Added: check for illegal vote command/argument - chrissstrahl
	//--------------------------------------------------------------
	if (strchr(command.c_str(), ';')	||
		strchr(arg.c_str(), ';')		||
		strchr(command.c_str(), '%')	||
		strchr(arg.c_str(), '%')		)
	{
