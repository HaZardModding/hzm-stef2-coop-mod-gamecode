...
void ModifierDiffusion::attachBomb( Player *player )
{
	...
	attachEvent->AddVector( player->upgPlayerGetBackpackAttachAngles() );
...
//hzm gamefix chrissstrahl - prevent chrash if map has no bomb present
...
void ModifierSpecialties::itemTouched( Player *player, MultiplayerItem *item )
{
	...
	//[b607] chrissstrahl - this was disabled, dono why. Standing on the item would spam with "...becomes a"
	_playerSpecialtyData[ player->entnum ]._specialty = specialty;
...
void ModifierSpecialties::setupSpecialty( Player *player, SpecialtyType specialty, bool chosen )
{
	...
	if ( !multiplayerManager.isPlayerSpectator( player ) )
	{
		if ( specialtyName.length() > 0 )
		{
			...
			//[b607] chrissstrahl - this reduandant and useless, so disable it (The classname of the player will be shown to all players anyway)
...
{
	//[b607] chrissstrahl - allow player to keep their class
	SpecialtyType stOldSpec = _playerSpecialtyData[killedPlayer->entnum]._specialty;
	
	putItemBack( killedPlayer );

	//[b607] chrissstrahl - allow player to keep their class
	_playerSpecialtyData[killedPlayer->entnum]._specialty = stOldSpec;
}
...
void ModifierSpecialties::playerSpawned( Player *player )
{
	...
	{
		//[b607] chrissstrahl - allow player to keep their class
		SpecialtyType stOldSpec = _playerSpecialtyData[player->entnum]._specialty;

		putItemBack( player );
		_playerSpecialtyData[ player->entnum ].reset();

		//[b607] chrissstrahl - allow player to keep their class
		if (stOldSpec > SPECIALTY_INFILTRATOR) {
			_playerSpecialtyData[player->entnum]._specialty = stOldSpec;			
		}
