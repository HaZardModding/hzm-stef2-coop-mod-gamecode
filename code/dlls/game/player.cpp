...
Player::~Player()
{
	...
	{
		//hzm gameupdate chrissstrahl - update vote if a player is leaving
		multiplayerManager.updateVotes( this , true );
		//hzm eof
...
void Player::Init( void )
{
	...
	//hzm gamefix chrissstrahl - disable viewmode when respawning, otherwise player remians in that viewmode, but without the hud or tricorder needed
	setViewMode( "none" );
	...
	if ( multiplayerManager.inMultiplayer() )
	{
		...
		//hzm gameupdate chrissstrahl - update vote if a player is joining
		multiplayerManager.updateVotes( this , false );
		//hzm eof
	}
	else
	{
...
void Player::Respawn( Event *ev )
{
	if ( multiplayerManager.inMultiplayer() )
	{
		//hzm coop mod chrissstrahl - another assert killed!
...
void Player::Dead( Event *ev )
{
	...
	else
	{
		...
		int i = ( rand() % 10 ) + 1;
		str playerKilled = va("PlayerKilled%d",i); //[b60011] chrissstrahl - added $$
...
{
	//hzm gamefix chrissstrahl - fix bug that speedboots and other speed changing stuff is no longer working
	//this is a issue that comes with the stock patch1.1 source code, but is not inside the actual patch 1.1
	//the upm mod also has that issue and it was never resolved there
	float fTemp = 0.0f;

	if ( _powerup ) {
		fTemp = ( float )moveSpeed * _powerup->getMoveMultiplier();
		moveSpeed = (int)fTemp;
	}

	if ( _rune ) {
		fTemp = ( float )moveSpeed * _rune->getMoveMultiplier();
		moveSpeed = ( int )fTemp;
	}
	//end of hzm gamefix
...
void Player::ClientMoveFlagsAndSpeeds( int moveSpeed , int noclipSpeed , int crouchSpeed , int airSpeed )
{
	...
	//hzm coop mod chrissstrahl - disable strafejumping in coop
	if ( sv_strafeJumpingAllowed->integer && !game.coop_isActive ){
...
void Player::ClientThink( Event *ev )
{
	...
	//hzm coop mod chrissstrahl - if coop, do not add to jumpvelocity, try to keep SP feeling
	...
	else
	{
		...
		if ( groundentity && groundentity->entity && groundentity->entity->isSubclassOf( Actor ) && !multiplayerManager.isPlayerSpectator( this ) )
		{
...
void Player::SetTargetedEntity( EntityPtr entity )
{
	...
	//  Shows archetype info on targeting
	//end of hzm
	if ( _targetedEntity != 0 ) {
		_targetedEntity->edict->s.eFlags |= EF_DISPLAY_INFO;
		...
		if ( ( lastSend + 1 ) < level.time ) {
			lastSend = ( int )level.time;
			if ( _targetedEntity != 0 ) {
				GameplayManager *gpm = GameplayManager::getTheGameplayManager();
				str type = _targetedEntity->getArchetype();
				//type += "Description1";
				...
				//this->hudPrint(va("1: $$%s$$\n", descr1.c_str() ) );
				//this->hudPrint( va( "2: $$%s$$\n" , descr2.c_str() ) );
				//this->hudPrint( va( "3: $$%s$$\n" , descr3.c_str() ) );
			}
		}
		//end of hzm
		*/
...
void Player::ProcessTargetedEntity( void )
{
	...
	//hzm coop mod chrissstrahl - make sure the archetypes work some how
	int i;
	Player* players;
	for ( i = 0; i < maxclients->integer; i++ )
	{
...
void Player::EvaluateState( State *forceTorso , State *forceLegs )
{
	...
	do
	{
		...
		if ( count > 10 )
		{
			//hzm gameupdate chrissstrahl - fixing game erroring out if weapon is added to broken list while being fired
			Weapon *weapon;
			...
			if ( world->findBrokenThing( weapon->getName() ) )
			{
				break;
			}
			//end of hzm
...
void Player::Kill( Event *ev )
{
	...
	//hzm gameupdate chrissstrahl - this fixes: kill working during cinematic/spectator mode or if already dead
	if ( level.cinematic || health <= 0.0f || multiplayerManager.isPlayerSpectator( this ) ) {
		return;
	}
	//end of hzm
...
void Player::ProcessPmoveEvents( int event )
{
	...
	case EV_FALL_FATAL:
		...
		//hzm coop mod chrissstrahl - take more falldamage in coop
		...
		//hzm coop mod chrissstrahl - make sure we will always get falldamage in coop
		if ( !multiplayerManager.checkFlag( MP_FLAG_NO_FALLING ) || game.coop_isActive )
		{
			Damage( world , world , damage , origin , vec_zero , vec_zero , 0 , DAMAGE_NO_ARMOR , MOD_FALLING );
		}
		break;

		//hzm gamefix, there is no such thing as a snd_fall
...
void Player::WorldEffects( void )
{
	...
	// check for drowning
	//
	//hzm gamefix - Commented the following code out, because drowning is handled in gamefix_WorldEffects now. BEWARE: Uncommenting this breaks drowning.
	/*
	if ( waterlevel == 3 )
	{
...
void Player::CalcBlend( void )
{
	...
	//hzm gamefix chrissstrahl - if player is in water and crouching he will now also have the aplha blend set
	if ( GetCrouch() )
	{
		vieworg[2] += 43;
	}
...
void Player::ShakeCamera( void )
{
	...
	//hzm coop mod chrissstrahl - show always damageshake in coop
	if ( damage_count && sv_showdamageshake->integer || damage_count && game.coop_isActive )
	{
		if ( health <= 0.0f )
		{
			damage_angles = vec_zero;
		}
		//hzm coop mod chrissstrahl - use singleplayer damageshake in coop
		else if ( multiplayerManager.inMultiplayer() && !game.coop_isActive )
		{
...
{
	Entity const *curTarget = GetTarget();
	if ( curTarget && targetEnemyLocked )
	{
		const Vector targetPoint( curTarget->centroid );
...
void Player::UpdateStats( void )
{
	...
	//hzm gameupdate chrissstahl - disabled the ifndef, because we want this to work on any kind of server
#ifndef DEDICATED // <--- COMMENTING THIS OUT MADE LINUX GO BOOM!	
	//
	// mission objectives
	//
	...
	if ( dualweapon )
	{
		...
		for ( i = 1; i <= ammo_inventory.NumObjects(); i++ )
		{
	...
	{
		ammo = NULL;
		ammo = ammo_inventory.ObjectAt( i );
...
void Player::UpdateMisc( void )
{
	...
	//hzm oop mod chrissstrahl - test TEST FAILD 2016.07.01 giving up
	//client->ps.dialogSoundIndex = 211;
	//client->ps.dialogTextSoundIndex = 211;
	
	//client->ps.dialogTextSoundIndex = G_FindConfigstringIndex( bossname->string , CS_GENERAL_STRINGS , MAX_GENERAL_STRINGS , true ) + CS_GENERAL_STRINGS;
		
	//hzm coop mod chrissstrahl - test, find what is causing the dialog text to flicker or zero out
	/*
	...
	if ( entitynum != _dialogEntnum || sndIndex != _dialogSoundIndex || textIndex != _dialogTextSoundIndex ){
		entitynum = _dialogEntnum;
		sndIndex = _dialogSoundIndex;
		textIndex = _dialogTextSoundIndex;
		gi.Printf( va( "COOP TEST DIALOG %i, %i, %i, ::%f\n" , _dialogEntnum , _dialogSoundIndex , _dialogTextSoundIndex , level.time ) );
	}
	*/
}
...
void Player::ArmorDamage( Event *ev )
{
	...
	//hzm coop mod chrissstrahl - Enable SKILL (Singleplayer dificulty) also for coop
	if ( !multiplayerManager.inMultiplayer() || game.coop_isActive && ( damage.meansofdeath != MOD_FALLING ) )
	{
		GameplayManager *gpm;
		float damageMultiplier;
		int skillLevel;

		//hzm gamefix chrissstrahl - grab skill directly
		if ( g_gametype->integer == GT_SINGLE_PLAYER )
		{
			skillLevel = getSkill();
		}
		...
		//bugfix on coop mod
		//hzm coop mod chrissstrahl - make sure that players do not do damage to each other based upon skill!
		if ( gpm->hasObject( "SkillLevel-PlayerDamage" ) && !damage.attacker->isSubclassOf( Player ) )
		{
			if ( skillLevel == 0 )
				damageMultiplier = gpm->getFloatValue( "SkillLevel-PlayerDamage" , "Easy" );
			else if ( skillLevel == 1 )
...
void Player::handleDialogSetup( Entity* entity , const str& soundName )
{
	...
	if ( gi.SoundLength( soundName.c_str() ) >= 0 )
	{
		...
		//test failed, giving up 2016.07.01
		//hzm coop mod chrissstrahl - see if we can get the right sound index
		//we want to keep it compatible for both languages
		//str sTemp = soundName;
		//if ( !coop_textReplace( sTemp , "loc/Eng/" , "localization/" ) ) {
			//coop_textReplace( sTemp , "loc/Deu/" , "localization/" );
		//}
		//_dialogSoundIndex = gi.soundindex( sTemp.c_str() );
		//_dialogTextSoundIndex = gi.soundindex( sTemp.c_str() );
		//gi.Printf( va( "======\n_dialogTextSoundIndex: %d\n%\n" , _dialogTextSoundIndex, soundName.c_str() ) );//hzm test temp
		//hzm eof
...
{
	//hzm coop mod chrissstrahl - crashed the clients in mp
	if ( g_gametype->integer == GT_SINGLE_PLAYER ) {
...
void Player::Score( Event *ev )
{
	...
	if (game.coop_isActive && this->health <= 0.0f && (this->coopPlayer.diedLast + 10) < level.time) {
...
void Player::CallVote( Event *ev )
{
	...
	if ( multiplayerManager.inMultiplayer() )
	{
		...
		int iArgsNum = 1;
		str sCombine = "";
		while (ev->NumArgs() > iArgsNum) {
			arg = ev->GetString( iArgsNum + 1 );
			if (iArgsNum > 1) {
				sCombine += " ";
			}
			sCombine += arg;
		...
		multiplayerManager.callVote( this , command , sCombine.c_str());
	}
}
...
void Player::setDisableUseWeapon( Event* ev )
{
	...
	//hzm gameupdate chrissstrahl - no one ever needs this message, so we disable it, it only spamms the console
	//Com_Printf( "DisableUseWeapon %d\n" , _disableUseWeapon );//hzm fixed the missing: \n
}
...
// Returns:		None
//-----------------------------------------------------
//hzm gameupdate chrissstrahl - we want to be able to disable weapons from anywhere in game code
void Player::disableUseWeapon( bool bDiable )
{
	if ( bDiable )
	{
		_disableUseWeapon = true;
...
void Player::hudPrint( const str &string )
{
	...
	//hzm gameupdate chrissstrahl - make hudprint sort of work in singleplayer too
	if ( g_gametype->integer != GT_SINGLE_PLAYER ){
		command = "hudprint ";
		//command = "hudprint ";
		command += string;
		...
		{
			gi.centerprintf( gentity , CENTERPRINT_IMPORTANCE_NORMAL , string.c_str() );
		}
	}
}
...
{
	//hzm coop mod chrissstrahl - make sure the vote string is not to long
	str s;
	s = voteText;
	//cut starting at $
...
{
	//[b608] chrissstrahl - I can't fix the mouse courser staying active
	//- if i force the curser to go away a player might get stuck in a menu
