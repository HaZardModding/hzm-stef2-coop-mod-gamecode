...
void RuneAmmoRegen::specificUpdate( float frameTime )
{
	...
	if ( _owner && ( level.time > _nextGiveTime ) && _owner->isSubclassOf( Player ) )
	{
		...
		if ( weapon )
		{
			...
			//[b607] chrissstrahl - that is by far to less ammo, give a bit more or the item is pretty much useless in multi
			//also regenerate ammo of other weapons, and make this item finally usefull
			...
			for (int i = 0; i < 3;i++) {
				...
				player->GiveAmmo(sType, iAmmount, false );
			}			
		}
...
bool HoldableItemExplosive::isPlayerInRange( const Vector &position, float maxDistance )
{
	...
	for( i = 0; i < game.maxclients; i++ )
	{
		...
		if ( entity == _owner || multiplayerManager.getPlayersTeam( ( Player* )entity ) == multiplayerManager.getPlayersTeam( ( Player* )_owner ) ) {
			continue;
		}
		//end of hzm
