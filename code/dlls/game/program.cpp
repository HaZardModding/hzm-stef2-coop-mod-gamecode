...
void Program::CreateDefForEvent( Event *ev )
{
	...
	if ( numfunctions >= MAX_FUNCTIONS )
	{
		...
		if ( coop_serverError( "Exceeded max functions while declaring events." , true ) ) {
...
int Program::AllocString()
{
	...
	//hzm coop mod chrissstrahl - quit complete game, allow reboot if server is dedicated
	if ( coop_serverError( "Program::GetFreeString : Too many strings allocated!" , false ) ) {
...
void Program::Load( const char *filename )
{
	...
	{
		//hzm coop mod chrissstrahl - quit complete game, allow reboot if server is dedicated
		if ( coop_serverError( "Level Script Compile failed" , true ) ) {
			gi.Error( ERR_DROP , "Level Script Compile failed: Please open console and scroll up\n" );
		}
		else {
			gi.Error( ERR_DROP , "Level Script Compile failed: Please open console and scroll up\n" );
		}
...
//hzm gameupdate daggolin - direct access to script-vars
