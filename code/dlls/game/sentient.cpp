...
void Sentient::DeactivateWeapon( weaponhand_t hand )
{
	...
	{
		//hzm coop mod chrissstrahl - nobody needs to know this nowdays, it is useles now
		...
		const Vector &normal, int knockback, int dflags, int meansofdeath, int surfaceNumber, int boneNumber, Entity *weapon , bool showPain )
{
	...
	if ( this->isSubclassOf( Player ) )
	{
		...
		//hzm coop mod chrissstrahl - also cut in half when we are in coop
		if ( !multiplayerManager.inMultiplayer() || game.coop_isActive && !player->groundentity)
			knockback*=0.5;
...
void Sentient::setViewMode(Event *ev)
{
	...
	//[b607] chrissstrahl - set state for script to detect viewmode
	if (this->isSubclassOf(Player)) {
		entityVars.SetVariable("_playerCurrentViewMode", ev->GetString(1));
	}
...
void Sentient::setViewMode( const str &viewModeName )
{
	...
	if ( _viewMode != newViewMode )
	{
		...
		//hzm coop mod chrissstrahl - fix message showing that you switched to mode none
		if ( this->isSubclassOf( Player ) )
		{
			//hzm gameupdate chrissstrahl - fix message changed to viewmode none (on level start)
			if ( viewModeName != "none" ){
...
//hzm coop daggolin - getSentient
WeaponPtr* Sentient::getActiveWeaponList()
{
	return this->activeWeaponList;
}
