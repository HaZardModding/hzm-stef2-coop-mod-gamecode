...
{
	//hzm gamefix chrissstrahl - returning any player, rather than just client 0
	Player *player;
	...
	if ( !player )
	{
		gi.WPrintf( "SoundManager_GetPlayer - No player found.\n" );
		return NULL;
	}
...
void SoundManager::PreviewReverbEvent( Event *ev )
{
	...
	//hzm gamefix chrissstrahl - returning any player, rather than just client 0
	Player *client = coop_returnPlayerFavored();
...
void SoundManager::Load( void )
{
	...
	//hzm coop mod chrissstrahl - try loading modified coop file if it exists
	filename = coop_serverModifiedFile( va( "maps/%s.snd" , filename.c_str() ) );
