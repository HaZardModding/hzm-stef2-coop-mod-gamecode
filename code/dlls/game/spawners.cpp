...
void Spawn::SetArgs( SpawnArgs &args )
{
	...
	//gi.Printf( va( "spawn effect: %s ($%s)\n" , _effectType.c_str(), this->targetname ) );
...
//hzm gameupdate chrissstrahl - allow use of beam effects!
		/* this is not working, try something else
		if ( stricmp( _effectType.c_str() , "TransportOutFederation" ) == 0 ){
			_effectName = "Federation";
		}
		else if ( stricmp( _effectType.c_str() , "TransportInFederation" ) == 0 ){
			_effectName = "Federation";
		}
		else if ( stricmp( _effectType.c_str() , "TransportInRomulan" ) == 0 ){
			_effectName = "Romulan";
		}
		else if ( stricmp( _effectType.c_str() , "TransportOutRomulan" ) == 0 ){
			_effectName = "Romulan";
		}*/
...
//hzm gameupdate chrissstrahl - removes this entity by using the cnt (count) command, once it hits its limit, see triggers for a example
	if ( count > -1 )
	{
		count--;
		if ( count < 1 )
