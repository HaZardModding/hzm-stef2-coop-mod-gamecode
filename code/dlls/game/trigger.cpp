...
//hzm coop mod chrissstrahl - use this to manage the cnt paramater in a simple function
void Trigger::RemoveSelfOnCountHit( void )
{
	if ( count > -1 )
	{
		count--;
...
//hzm gameupdate chrissstrahl - get threadname from trigger
str	Trigger::GetThread( void )
{
	str sThreadname = "";
	sThreadname = thread;
	return sThreadname;
}
//hzm gameupdate chrissstrahl - set threadname on trigger
void Trigger::SetThread( str threadname )
{
	thread = threadname;
}
//hzm gameupdate chrissstrahl - get actitating entity
EntityPtr Trigger::GetLastActivatingEntity()
{
...
void Trigger::TriggerStuff( Event *ev )
{
	...
	/*
	//hzm gamefix chrissstrahl - fix triggerhurt usable by players pressing use! Trigger hurt shall only respond to player EV_Touch
	if ( other->isSubclassOf( Player ) && !Q_stricmp( this->getClassname() , "TriggerHurt" ) && ( int )*ev == ( int )EV_Use )
		return;
	*/
...
void Trigger::ActivateTargets( Event *ev )
{
	...
	if ( message.length() && other && ( other->isClient() || other->isSubclassOf( Camera ) ) )
	{
		...
		if ( !other->isClient() )
		{
			//hzm gameupdate chrissstrahl - make work also in mp
			Entity *ePlayer = coop_returnEntityPlayerFavored();
			//end of hzm
			//gi.centerprintf( &g_entities[ 0 ], CENTERPRINT_IMPORTANCE_NORMAL, message.c_str() );
			gi.centerprintf( ePlayer->edict , CENTERPRINT_IMPORTANCE_NORMAL , message.c_str() );
		}
		else
		{
			//gi.centerprintf( other->edict, CENTERPRINT_IMPORTANCE_NORMAL, message.c_str() );
			
			//hzm gameupdate chrissstrahl - print to hud instead
			if ( g_gametype->integer != GT_SINGLE_PLAYER )
			{
...
TriggerSecret::TriggerSecret()
{
	...
	//hzm gamefix chrissstrahl - relict, not used in ef2, so it has now been dissabled, creates a warning each time in the console
...
{
	//hzm gamefix chrissstrahl - fix trigger hurt responding to event use
	{ &EV_Use, NULL } ,
	//end of hzm
...
void TriggerHurt::Hurt( Event *ev )
{
	...
	//hzm gamefix chrissstrahl - this would crash the server
	//if a player should get hurt by a trigger but leaves the server just in the same frame
	if ( other == NULL ){
		return;
	}
	...
	//hzm gamefix chrissstrahl - hurt all players that are inside the trigger field, not just the one who activated it 
	if ( g_gametype->integer == GT_SINGLE_PLAYER )
		return;

	//hzm chrissstrahl - i know i should check for actors and players entities (actors and players but this is more efficent and i doubt anyone will ever notice)
	int i;
	Entity *entity2 = NULL;
	...
	for ( i = 0; i < maxentities->integer; i++ ) {
		...
		{
			entity2->Damage( this , world , damage , entity2->origin , vec_zero , vec_zero , 0 , DAMAGE_NO_ARMOR | DAMAGE_NO_SKILL , damage_type );
		}
	}
	//end of hzm
}
...
void TriggerCallVolume::CheckReady( Event *ev )
{
	...
	//gi.Printf( "TriggerCallVolume::CheckReady\n" );
	
	//hzm coop mod chrissstrahl - this is for singleplayer
	if ( g_gametype->integer == GT_SINGLE_PLAYER )
	{
		//gi.Printf( "TriggerCallVolume::CheckReady: SP\n" );
		player = GetPlayer( 0 );
		...
		//gi.Printf( "TriggerCallVolume::CheckReady: SP OK\n" );
	}
	else
	{
		//gi.Printf( "TriggerCallVolume::CheckReady: mp START\n" );
		...
		//gi.Printf( "TriggerCallVolume::CheckReady: mp OK\n" );
	}
	...
	//gi.Printf( "TriggerCallVolume::CheckReady: ALL OK - CECK SUCESSFULL\n" );
}
...
{
	//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts\n" );
	...
	Player *player;
	//hzm coop mod chrissstrahl - grab a valid player in sp
	if ( g_gametype->integer == GT_SINGLE_PLAYER )
	{
		player = GetPlayer( 0 );
		player->SetCurrentCallVolume( TargetName() );
		//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts SP\n" );
	}
	//hzm coop mod chrissstrahl - grab a valid player in multiplayer
	else
	{
		//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts MP\n" );
		int i;
		Player *temp = NULL;
		...
		for ( i = 0; i < maxclients->integer; i++ )
		{
			if ( g_entities[i].inuse && g_entities[i].entity && g_entities[i].client )
			{
				...
				{
					temp->SetCurrentCallVolume( TargetName() );
				}
			}
		}
	}
	...
	//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts OBJECTS\n" );
	
	for ( int i = 1; i <= _requiredEntities.NumObjects() ; i++ )
	{
		//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts OBJECT %i\n", i );
		name = _requiredEntities.ObjectAt( i );
		ent = _getEntity( name );
		//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts OBJECT %s\n" , name.c_str() );
		if ( ent && ent->isSubclassOf(Actor ) )
		{
			//gi.Printf( "TriggerCallVolume::_notifyRequiredEnts SETFLAG %s\n" , name.c_str() );
			actor = (Actor*)ent;
