...
void ViewMaster::Shoot( Event *ev )
{
	...
	Player* player = coop_returnPlayerFavored();
	if ( !player ) return;
...
void ViewMaster::PullToCamera( Event* )
{
	...
	//hzm coop mod chrissstrahl - return any player
	// Check if we have a client
	Entity* clientEntity = coop_returnEntityPlayerFavored();
...
void ViewMaster::Spawn( Event *ev )
{
	...
	//hzm coop mod chrissstrahl - grab any player
	// Check if we have a client
	ent = coop_returnEntityPlayerFavored();
	if ( !ent )
	{
		return;
	}
