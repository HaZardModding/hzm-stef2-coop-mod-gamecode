...
//hzm coop mod chrissstrahl - animate weapon holster during coop
	if ( owner && owner->isSubclassOf( Actor ) || game.coop_isActive )
	{
...
//hzm coop mod chrissstrahl - do not drop weapon while in coop and player is dead
	if (!IsDroppable() || owner->getHealth() < 1 && game.coop_isActive )
	{
		return false;
	}
	...
	if ( owner )
	{
		//hzm coop mod chrissstrahl - test if the weapon does not get trown 
	...
	//hzm coop mod chrissstrahl - keep ammo in coop
	if ( !game.coop_isActive )
	{
		for ( i = 0; i < MAX_FIREMODES; i++ )
		{
	...
	*/
	...
	//hzm coop mod chrissstrahl - wait a little longer during coop, before we fade out the weapon
	if ( game.coop_isActive ){
		PostEvent( EV_FadeOut , 120.0f );
	}else{
