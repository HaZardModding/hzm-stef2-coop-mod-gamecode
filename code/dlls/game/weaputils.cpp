...
void Projectile::Touch( Event *ev )
{
	...
	if ( ( projFlags & P_BOUNCE_TOUCH ) && !other->takedamage )
		//( !other->takedamage || other->health <= 0.0f ) )
	{
	...
	if ( realname.length() > 1 )
		//hzm gamefix chrissstrahl - fix bat'leth stabbing sound being played global
		Sound( realname, CHAN_BODY );//Sound( realname , CHAN_VOICE );
		//end of hzm
	// 
	// Override damage if we're using the GameplayManager system
	//
	...
	Entity	*weap )
{
	...
	if (ent && ent->takedamage) {
		if (ent->isSubclassOf(ScriptSlave) || ent->isSubclassOf(ScriptModel)) {
			...
			if (entScriptSlave->upgScriptSlaveGetDamageLabel().length() || entScriptSlave->GetTriggerOnDamage() ) {
				objectHit = true;
			}
		}
	...
	if ( damage_total > 0.0f || objectHit )
	{
		...
		{
			//--------------------------------------------------------------
			// [b60025] GAMEFIX - Fixed: Warning C4456: Declaration of player hides previous local declaration. - chrissstrahl
			//--------------------------------------------------------------
